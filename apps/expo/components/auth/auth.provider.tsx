...
export const AuthProvider = observer(({ children }: { children: JSX.Element }) => {
  const segments = useSegments();
  console.log(`Navigated to ${segments.join("/")}`);
  const publicRoute =
    AuthStore.publicRoutes.some((publicRoute) => (segments as string[]).includes(publicRoute)) || segments.length === 0;
  useEffect(() => {
    if (!AuthStore.loaded) {
