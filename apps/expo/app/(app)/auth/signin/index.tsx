...
import { Button, Heading, ScrollView, Text, View, XStack, YStack } from 'tamagui'
        ...
        <Formik
          ...
          validate={(values) => {
            ...
            if (!values.email) {
              ...
              errors.password = translate.t('auth.login.errors.password.required')
            }
            return errors
          }}
              ...
              if (error instanceof AuthError) {
                if (error?.name === 'USER_NOT_CONFIRMED_ERROR') {
                  try {
                    ...
                    router.push(
                      ('/auth/email/verify-sent' + `?mail=${values.email}`) as Href<unknown>
                    )
                  } catch (errorTmp) {
                    setErrors({
              ...
              <YStack space="$2">
                <XStack justifyContent={'space-between'} alignItems="center">
                  ...
                  <Link href="/auth/password/reset/">
                    <Text color="$gray10">{translate.t('auth.login.forgotPassword')}</Text>
                  </Link>
                </XStack>
              </YStack>
              <View>
